...
void InitShop() // OK
{
	//SetCompleteHook(0xE9,0x005EAC9B,&RenderStackNumber);

	SetCompleteHook(0xE9,0x005B0510,&GetItemDurability);

	SetCompleteHook(0xE9,0x005AFD00,&GetItemRepairMoney);

	SetCompleteHook(0xE9,0x00537D28,&GetItemValue);

	SetCompleteHook(0xFF,0x005B254D,&ShowBuyPrice);

	SetCompleteHook(0xE9,0x005B2687,&ColorBuyPrice);
}

__declspec(naked) void RenderStackNumber() // OK
{
    static DWORD ExitAddress = 0x005EACA0;
    static DWORD NextAddress = 0x005EACAF;
	static DWORD lpItem = 0;
	...
	if(ItemMaxStack(*(WORD*)lpItem,((*(DWORD*)(lpItem + 2) >> 3) & 15)) != 0 && *(BYTE*)(lpItem + 22) > 1 && (*(WORD*)lpItem != GET_ITEM(4,7) && *(WORD*)lpItem != GET_ITEM(4,15)))
	{
		_asm
		{
			PopAd
...
DWORD GetItemValue(DWORD address,int type) // OK
{
	...
	if(ShopValueType != 0)
	{
		int x = *(BYTE*)(address + 56);

		int y = *(BYTE*)(address + 57);
	...
	else
	{
		...
		else
		{
			...
			for(int n = 0; n < *(BYTE*)(address + 37); n++)
			{
				if(*(BYTE*)(address + 38 + n) == 84)
				{
					price += (price*25)/100;
					break;
				}
...
{
	static DWORD ExitAddress1 = 0x005B268C;
	static DWORD ExitAddress2 = 0x005B2690;

	_asm
	{
		Mov Eax,Dword Ptr Ds:[0x07672F38]
		Cmp ShopValueType,0
		Jnz Next
		Jmp[ExitAddress1]
	Next:
